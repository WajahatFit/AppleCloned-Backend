  ...
  async (req, res, next) => {
      ...
      try {
          ...
          if (cartInDB.length < 1) {
              const cartFromDB = await Cart.create({ products: [productId], user: _id })
              res.status(200).json({data: cartFromDB})
          } else {
              const updatedCart = await Cart.findOneAndUpdate({ user: _id }, { $push: { products: productId } }, {new: true})
...
router.put('/delete/:id', isAuthenticated, async (req, res, next) => {
    const { productId } = req.params;
    const { user } = req.payload;
    try {
        const product = await Product.findById(productId);
        const prevCart = await Cart.findOne({ user: user._id });
        console.log(proprevCart.products)
